...
% Channel 1
idx1         = find(abs(audioData(1,:)) >= threshold);
...
% Channel 2
idx2         = find(abs(audioData(2,:)) >= threshold);
if ~isempty(idx2) % Excute only if peaks were found
    i            = 1;
    narrowWindow = 0; % Used for correcting if the time window is larger than idx1
    if i + timeWindowIdx > length(idx2)% If the index exceeds the idx1 vector make window smaller
